...
class FlashbackWindow: UIWindow {
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        if (point.x < config.triggerRange && config.enablePositions.contains(.left)) || (point.x > self.bounds.width - config.triggerRange && config.enablePositions .contains(.right)) {
