...
@objc public class FlashbackManager: NSObject {
    ...
    public var config: FlashbackConfig = .default {
        didSet {
            self.flashbackView.config = config
        }
    }
    ...
    /// 指示器视图
    private lazy var flashbackView = FlashbackView()
    ...
    /// 把手势返回提到最上层
    @objc
    public func makeFlashbackToTop() {
        if self.flashbackView.superview != nil {
            self.targetWindow?.bringSubviewToFront(self.flashbackView)
        }
    }
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if self.isEnable {
                if let targetWindow = self.targetWindow {
                    self.flashbackView.frame = targetWindow.frame
                    targetWindow.addSubview(self.flashbackView)
                    self.monitorKeyboard(true)
                    self.monitorDeviceOrientation(true)
                    self.makeFlashbackToTop()
                }else {
                    fatalError("targetWindow can not be nil")
                }
            } else {
                if self.flashbackView.superview != nil {
                    self.flashbackView.removeFromSuperview()
                }
    ...
    /// 监控键盘
    private func monitorKeyboard(_ isMonitor: Bool) {
        let notify = NotificationCenter.default
        if isMonitor {
